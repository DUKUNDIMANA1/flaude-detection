...
class User(db.Model):
    ...
    # ── Relationships ─────────────────────────────────────────────────────────
    # Transaction has TWO FKs to users (user_id + reviewed_by) → must be explicit
    ...
    # Alert has TWO FKs to users (user_id + resolved_by) → must be explicit
...
class Transaction(db.Model):
    ...
    # Alerts linked to this transaction
    alerts = db.relationship(
        'Alert',
        foreign_keys='Alert.transaction_id_ref',
        backref='transaction',
        lazy=True,
    )
